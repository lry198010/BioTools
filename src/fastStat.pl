...
$fileName =~ s/\.[^\.]*$//;
$fileBase = "$filePath$fileName" if $fileBase eq "";

print STDERR "Note: Fasta file:$fasta\n";
if ($cluster) {
  print STDERR "Note: Cluster file:$cluster\n";
} else {
  print STDERR "Note: No cluster file assigned, corset genes stat will not perform\n";
}
print STDERR "Note: minimul len to stat:$minLen\n";
print STDERR "Note: the minimul value of transcript/gene/unigene length of histogram: $begin\n";
print STDERR "Note: the interval value of transcript/gene/unigene length of histogram: $group_len\n";
print STDERR "Note: the maximul value of transcript/gene/unigene length of histogram: $end\n";
print STDERR "Note: whether perform length stat for genes extracted from trinity assembly information[1 for yes]: $doTrinity\n";
...
my $groups = getGroupRange($begin, $end, $group_len);
...
if ($doTrinity) {
  ...
  $groups = getGroupRange($begin, $end, $group_len);
...
if ($doCor) {
  ...
  %corsetGenes = corsetGenes($cluster,%seq);
  ...
  print STDERR "Corset Genes: Sequence length output to file: $fileBase.corgene.len\n";


  %corsetGenes = genes2trans("Corset genes",%corsetGenes);
}

if ($doTrinity + $doCor <= 0 ) {
  print STDERR "ok finished\n";
  exit(0);
}

print STDERR ("Trinity Genes: Sequence output to file: $fileBase.trigene.fa\n") if $doTrinity;
print STDERR ("Corset Genes: Sequence output to file: $fileBase.corgene.fa\n") if $doCor;
      ...
      if($txt =~ /^>(\S+)/){
        if ($doTrinity) {
          if (defined $trinityGenes{$nowSeqId}){
            print TRIOUT ">",$trinityGenes{$nowSeqId}," $seqDesc\n";
            print TRIOUT $seq,"\n";
          }
        }
        if ($doCor) {
          if (defined $corsetGenes{$nowSeqId}){
            print COROUT ">",$corsetGenes{$nowSeqId}," $seqDesc\n";
            print COROUT $seq,"\n";
          }
        }
        $seq = "";
...
sub prtHelp {
  ...
  print STDERR "  -b <the minimul sequence length used to group the sequence:0>\n";
  print STDERR "    the minimul sequence length used to group the sequence\n";
  print STDERR "  -g <step length used to group the sequence:100>\n";
  print STDERR "    step length used to group the sequence\n";
  print STDERR "  -e <the maximul sequence length used to group the sequence:2000>\n";
  print STDERR "    the maximul sequence length used to group the sequence\n";
  print STDERR "\n";
